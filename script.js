...
function exportToExcel() {
  ...
  const table = document.querySelector('.table.table-responsive.table-condensed.table-striped.pri-table'); 
  ...
  const competencia = document.querySelector('p strong').textContent.trim(); 

  XLSX.utils.sheet_add_aoa(ws, [[mainTitle]], { origin: 'A1' });
  XLSX.utils.sheet_add_aoa(ws, [[subtitle]], { origin: 'A2' });
  XLSX.utils.sheet_add_aoa(ws, [[competencia]], { origin: 'A3' });
  XLSX.utils.sheet_add_aoa(ws, [tableHeaders], { origin: 'A5' }); 

  tableData.forEach((rowData, rowIndex) => {
    XLSX.utils.sheet_add_aoa(ws, [rowData], { origin: `A${rowIndex + 6}` });
  });
...
function exportToPDF() {
  ...
  doc.text(competencia, titleX, titleY + 16, { align: 'center' });
  doc.addImage(logoImg, 'PNG', logoX, logoY, logoWidth, logoHeight); 

  // Renderiza a tabela apenas uma vez
  const table = document.querySelector('.table.table-responsive.table-condensed.table-striped.pri-table');
  doc.autoTable({
    html: table,
    startY: 60,
    ...
    didDrawPage: function (data) {
      if (data.pageNumber > 1) {
        ...
        doc.text("RL01 - RELATÓRIO GERENCIAL", 10, 18);
        doc.text(competencia, 10, 26);
      }
    },
    didParseCell: function(data) {
      // Aplica negrito a células que contêm "<strong>" no HTML original
      if (data.section === 'body' && data.cell.raw.includes('<strong>')) {
